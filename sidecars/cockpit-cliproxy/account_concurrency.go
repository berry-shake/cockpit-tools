...
func (s *accountSlotSelector) Pick(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*coreauth.Auth) (*coreauth.Auth, error) {
	...
	for len(remaining) > 0 {
		...
		if err != nil || auth == nil {
			// At least one eligible account was full. Remaining candidates may
			// be outside the API-key scope or blocked by other routing rules;
			// keep this a capacity wait instead of losing the eligible pool.
			if len(excluded) > 0 && ctx.Err() == nil {
...
func (s *accountConcurrencySelector) Pick(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*coreauth.Auth) (*coreauth.Auth, error) {
	...
	if err != nil {
		if errors.Is(err, errAccountConcurrencyFull) {
			return s.waitForAnyAccount(ctx, provider, model, opts, auths, requestID, maxConcurrency)
		}
		return nil, err
	}
	if auth == nil {
	...
	return s.waitForAnyAccount(ctx, provider, model, opts, []*coreauth.Auth{auth}, requestID, maxConcurrency)
}

// waitForAnyAccount 等待可用槽位，每次重试仍经过完整选择器链，保留账号范围、
// 模型排除、额度和会话亲和规则；不能直接从未经筛选的候选列表占位。
func (s *accountConcurrencySelector) waitForAnyAccount(
	...
	for {
		...
		auth, err := s.fallback.Pick(ctx, provider, model, opts, candidates)
		if err != nil && !errors.Is(err, errAccountConcurrencyFull) {
			return nil, err
		}
		if err == nil && auth == nil {
			return nil, nil
		}
		if err == nil && s.tracker.tryReserveAccountSlot(requestID, auth.ID, maxConcurrency) {
			return auth, nil
		}
