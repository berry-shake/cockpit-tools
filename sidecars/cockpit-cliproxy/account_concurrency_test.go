...
func TestAccountConcurrencyWaitPreservesModelExclusions(t *testing.T) {
	tracker := newRequestUsageTracker()
	if !tracker.tryReserveAccountSlot("in-flight", "auth-a", 1) {
		t.Fatal("expected initial reservation")
	}
	m := &manifest{MaxAccountConcurrency: 1}
	selector := buildCoreAuthSelectorWithConcurrency(nil,
		&orderedAuthSelector{order: []string{"auth-a", "auth-b"}}, m, nil, tracker)
	allowed, excluded := testAuth("auth-a"), testAuth("auth-b")
	excluded.Metadata = map[string]any{"excluded_models": []string{"gpt-5.4"}}
	selected, err := selector.Pick(accountConcurrencyRequestContext("queued-request"),
		"codex", "gpt-5.4", cliproxyexecutor.Options{}, []*coreauth.Auth{allowed, excluded})
	var authErr *coreauth.Error
	if selected != nil || !errors.As(err, &authErr) || authErr.Code != "account_concurrency_exceeded" {
		t.Fatalf("full eligible account must not fall back to an excluded account: selected=%v err=%v", selected, err)
	}
	if got := tracker.accountInFlightCount(excluded.ID); got != 0 {
		t.Fatalf("excluded account reserved %d slots", got)
	}
}

func TestAccountConcurrencyWaitPreservesAPIKeyScope(t *testing.T) {
	tracker := newRequestUsageTracker()
	if !tracker.tryReserveAccountSlot("in-flight", "auth-a", 1) {
		t.Fatal("expected initial reservation")
	}
	allowed := &accountSpec{ID: "account-a", AuthID: "auth-a"}
	outsideScope := &accountSpec{ID: "account-b", AuthID: "auth-b"}
	m := &manifest{
		MaxAccountConcurrency: 1, AccountConcurrencyWaitMs: 1000,
		accountByAuthID: map[string]*accountSpec{"auth-a": allowed, "auth-b": outsideScope},
	}
	selector := buildCoreAuthSelectorWithConcurrency(nil, &cockpitSelector{manifest: m}, m, nil, tracker)
	ctx := context.WithValue(accountConcurrencyRequestContext("queued-request"),
		clientAPIKeyContextKey, &apiKeySpec{AccountIDs: []string{"account-a"}})
	timer := time.AfterFunc(40*time.Millisecond, func() { tracker.releaseAccountSlots("in-flight") })
	t.Cleanup(func() { timer.Stop() })
	selected, err := selector.Pick(ctx, "codex", "gpt-5.4", cliproxyexecutor.Options{},
		[]*coreauth.Auth{testAuth("auth-a"), testAuth("auth-b")})
	if err != nil || selected == nil || selected.ID != "auth-a" {
		t.Fatalf("wait must select the scoped account after release: selected=%v err=%v", selected, err)
	}
	if got := tracker.accountInFlightCount("auth-b"); got != 0 {
		t.Fatalf("out-of-scope account reserved %d slots", got)
	}
}

func TestAccountConcurrencyCancelledRequestDoesNotReserveSlot(t *testing.T) {
	tracker := newRequestUsageTracker()
	selector := buildCoreAuthSelectorWithConcurrency(nil, &orderedAuthSelector{},
		&manifest{MaxAccountConcurrency: 1}, nil, tracker)
	ctx, cancel := context.WithCancel(accountConcurrencyRequestContext("cancelled-request"))
	cancel()
	selected, err := selector.Pick(ctx, "codex", "gpt-5.4", cliproxyexecutor.Options{},
		[]*coreauth.Auth{testAuth("auth-a")})
	if selected != nil || !errors.Is(err, context.Canceled) {
		t.Fatalf("expected cancellation, selected=%v err=%v", selected, err)
	}
	if got := tracker.accountInFlightCount("auth-a"); got != 0 {
		t.Fatalf("cancelled request reserved %d slots", got)
	}
}
...
func TestAccountConcurrencyGateThroughAuthManager(t *testing.T) {
	...
	// Keep the fixture unexpired: this test exercises routing, not the
	// independent OAuth auto-refresh worker or its persistence writes.
