...
mod history_sanitize_scope_tests {
    #[test]
    fn switching_and_launching_do_not_repeat_history_migration() {
        let scan = concat!("sanitize_official_incompatible_", "reasoning_history(");
        assert!(!include_str!("codex_account_commands.rs").contains(scan));
        assert!(!include_str!("codex_instance.rs").contains(scan));
    }
}
