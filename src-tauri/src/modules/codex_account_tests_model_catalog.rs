...
// 测试与生产实现共享 super 作用域，验证真实持久化和运行态行为。
    ...
    #[test]
    fn deepseek_overrides_upgrade_legacy_backup_without_losing_original_values() {
        let base_dir = make_temp_dir("codex-deepseek-legacy-backup-upgrade");
        let backup_path = super::deepseek_compaction_backup_path(&base_dir);
        fs::write(
            &backup_path,
            r#"{"remote_compaction_v2":{"present":true,"value":true},"token_budget":{"present":false}}"#,
        )
        .expect("write v1.3.50 backup");
        let mut doc = crate::modules::codex_config_format::read_codex_config_doc_from_str(
            "web_search = \"live\"\nservice_tier = \"priority\"\n[features]\nremote_compaction_v2 = false\ntoken_budget = true\n",
        )
        .expect("parse active DeepSeek config");

        super::apply_deepseek_config_overrides(&mut doc, &base_dir);
        assert_eq!(doc["web_search"].as_str(), Some("disabled"));
        assert!(doc.get("service_tier").is_none());
        let upgraded = fs::read_to_string(&backup_path).expect("read upgraded backup");
        super::apply_deepseek_config_overrides(&mut doc, &base_dir);
        assert_eq!(fs::read_to_string(&backup_path).expect("read repeated backup"), upgraded);

        assert!(super::restore_deepseek_config_overrides(&mut doc, &base_dir));
        assert_eq!(doc["web_search"].as_str(), Some("live"));
        assert_eq!(doc["service_tier"].as_str(), Some("priority"));
        assert_eq!(doc["features"]["remote_compaction_v2"].as_bool(), Some(true));
    ...
    fn deepseek_overrides_do_not_change_config_when_backup_cannot_be_created() {
        ...
        fs::write(&blocked_dir, "block backup directory creation").expect("create blocker");
        let mut doc = crate::modules::codex_config_format::read_codex_config_doc_from_str(
            "web_search = \"live\"\nservice_tier = \"priority\"\n",
        )
        .expect("parse config");
        let before = doc.to_string();
        super::apply_deepseek_config_overrides(&mut doc, &blocked_dir);
        assert_eq!(doc.to_string(), before);
        fs::remove_dir_all(&base_dir).expect("cleanup temp dir");
    }

    #[test]
    fn deepseek_overrides_preserve_unreadable_backup_and_config() {
        let base_dir = make_temp_dir("codex-deepseek-invalid-backup");
        let backup_path = super::deepseek_compaction_backup_path(&base_dir);
        fs::write(&backup_path, "invalid backup").expect("write invalid backup");
        let mut doc = crate::modules::codex_config_format::read_codex_config_doc_from_str(
            "web_search = \"live\"\nservice_tier = \"priority\"\n",
        )
        .expect("parse config");
        ...
        super::apply_deepseek_config_overrides(&mut doc, &base_dir);
        assert_eq!(doc.to_string(), before);
        assert_eq!(fs::read_to_string(backup_path).expect("read backup"), "invalid backup");
        fs::remove_dir_all(&base_dir).expect("cleanup temp dir");
    }
