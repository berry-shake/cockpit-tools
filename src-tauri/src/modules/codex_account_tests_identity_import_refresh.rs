...
fn build_auth_file_value_writes_empty_refresh_token_when_account_has_none() {
    ...
    assert_eq!(
        auth_file.get("auth_mode").and_then(serde_json::Value::as_str),
        Some("chatgpt")
    );
...
fn build_auth_file_value_writes_official_auth_modes_without_custom_type() {
    ...
    assert_eq!(
        oauth_file.get("auth_mode").and_then(serde_json::Value::as_str),
        Some("chatgpt")
    );
    assert!(oauth_file.get("type").is_none());
    assert!(oauth_file.get("personal_access_token").is_none());
    assert_eq!(
        oauth_file.get("OPENAI_API_KEY").and_then(serde_json::Value::as_str),
    ...
    assert_eq!(
        pat_file.get("auth_mode").and_then(serde_json::Value::as_str),
