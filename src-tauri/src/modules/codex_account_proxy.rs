...
pub fn configured_url(account: &CodexAccount) -> Result<Option<Cow<'_, str>>, String> {
    resolve_checked(
        account,
        crate::modules::codex_unified_proxy::effective_snapshot,
    )
}
...
mod tests {
    ...
    fn account_binding_wins_and_unified_is_fallback() {
        ...
        assert_eq!(
            account.egress_proxy_url.as_deref(),
            Some("http://127.0.0.1:8080")
        );
        assert_eq!(
            resolve_effective(&account, None).as_deref(),
    ...
    fn shared_read_failures_never_become_an_unbound_account() {
        let mut account = account();
        assert_eq!(
            resolve_checked(&account, || Err("UNIFIED_PROXY_STORAGE".into())),
            Err("UNIFIED_PROXY_STORAGE".into())
        );
        account.egress_proxy_url = Some("http://127.0.0.1:8080".into());
        let own = resolve_checked(&account, || {
            panic!("independent binding must not read shared state")
        });
