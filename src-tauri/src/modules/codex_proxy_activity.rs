...
fn parse_log(value: &Value, channel: &'static str) -> Option<ProxyLogEntry> {
    let level = value["level"].as_str().or_else(|| value["type"].as_str())?;
...
pub fn attach_if_enabled(account_id: &str, channel: &'static str, controller: EngineController) {
    ...
    let handle = tokio::spawn(stream_logs(
        account_id.to_owned(),
        channel,
        controller.clone(),
    ));
    capture
        .tasks
        .insert(controller.tunnel_id, CaptureTask { controller, handle });
}
...
pub async fn set_enabled(account_id: String, enabled: bool) -> Result<(), String> {
    ...
    join_all(
        controllers
            .iter()
            .map(|controller| set_log_level(controller, "warning")),
    )
    .await;
    if enabled {
    ...
    Ok(value["connections"]
        ...
        .filter_map(|entry| {
            ...
            Some(ProxyConnection {
                ...
                channel,
                started_at: safe_field(entry["start"].as_str().unwrap_or(""), 80)
                    .unwrap_or_default(),
...
pub async fn snapshot(account_id: String) -> Result<ProxyActivitySnapshot, String> {
    ...
    let (enabled, capture_error, logs) = {
        ...
        captures
            .get(&account_id)
            .map_or((false, false, Vec::new()), |capture| {
                (
                    capture.enabled,
                    capture.error,
                    capture.logs.iter().rev().cloned().collect(),
                )
            })
    };
    let controllers = codex_proxy_runtime::active_controllers(&account_id).await;
    ...
    for result in join_all(
        controllers
            .into_iter()
            .map(|(channel, controller)| connection_list(channel, controller)),
    )
    .await
    {
        match result {
...
async fn account_summary(account: CodexAccount) -> ProxyActivitySummaryEntry {
    ...
    if codex_proxy_runtime::ensure_account_proxy_state(&account)
        .await
        .is_err()
    ...
    let responses = join_all(
        controllers
            .into_iter()
            .map(
                |(_channel, controller)| async move { connection_payload(&controller).await.ok() },
            ),
    )
    .await;
...
mod tests {
    ...
    fn only_route_fields_or_fixed_errors_escape_the_parser() {
        ...
        assert!(parse_log(
            &serde_json::json!({"type":"info","payload":"token=secret"}),
            "account"
        )
        .is_none());
        let error = parse_log(
            &serde_json::json!({"level":"error","message":"handshake failed token=secret"}),
            "desktop",
        )
        .unwrap();
    ...
    fn unsupported_or_idle_accounts_report_zero_without_errors() {
        ...
        let unsupported =
            summary_entry("api-key-account", false, false, ConnectionTotals::default());
    ...
    fn summary_aggregates_bytes_and_connection_counts() {
        ...
        let desktop_engine =
            serde_json::json!({"connections": [{"id":"c","upload":1,"download":2}]});
        let (readable, totals) =
        ...
        let maxed =
            serde_json::json!({"connections": [{"upload": u64::MAX, "download": u64::MAX}]});
