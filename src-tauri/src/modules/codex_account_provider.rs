...
fn write_deepseek_compaction_backup(
    ...
    let content = serde_json::to_string_pretty(&serde_json::Value::Object(backup.clone()))
        .map_err(|error| format!("序列化 DeepSeek 配置备份失败: {error}"))?;
...
pub(crate) fn apply_deepseek_config_overrides(doc: &mut Document, base_dir: &Path) {
    ...
    let mut original = read_deepseek_compaction_backup(base_dir);
    if backup_path.exists() && original.is_empty() {
        logger::log_warn("[DeepSeek] 配置备份无法读取，跳过临时配置改动以保留恢复能力");
        return;
    }
    let mut needs_backup = !backup_path.exists();
    ...
    // 旧备份可能缺整个分区，或仅缺新版新增键（例如 preferred_auth_method）。
    // 逐键补录，保留已经记录过的用户原值。
    let recorded = original.entry(DEEPSEEK_TOP_LEVEL_BACKUP_SECTION.to_string())
        .or_insert_with(|| serde_json::json!({}));
    let Some(recorded) = recorded.as_object_mut() else {
        logger::log_warn("[DeepSeek] 顶层配置备份格式无效，跳过临时配置改动");
        return;
    };
    if let Some(current) = record_deepseek_top_level_backup(doc).as_object() {
    ...
    if needs_backup {
        if let Err(error) = write_deepseek_compaction_backup(base_dir, &original) {
            logger::log_warn(&format!("[DeepSeek] 配置备份失败，跳过临时配置改动: {error}"));
            return;
        }
    }
