...
pub fn options_view(catalog: &ParsedCatalog) -> Option<Value> {
    ...
    if let Some(url) = group.url.as_deref().or_else(|| {
        native
    ...
    if let Some(interval) = group.interval.or_else(|| {
        native
    ...
    let tolerance = group.tolerance.map(u64::from).or_else(|| {
        native
...
pub fn build(
    ...
    let group = ParsedGroup {
        ...
        tolerance: (kind == "url-test").then(|| resolved.tolerance.unwrap_or(DEFAULT_TOLERANCE)),
        error: None,
...
mod tests {
    ...
    fn encoded_group(
        ...
        let raw =
            binding::encode("strategy-source", "Strategy", &group, catalog, selections).unwrap();
    ...
    fn every_strategy_kind_encodes_into_a_mihomo_group_with_options() {
        ...
        for kind in KINDS {
            let catalog = build("Strategy", kind, &options, "group-id".into(), nodes()).unwrap();
            ...
            // 超时按秒保存，写入内核配置时换算成毫秒。
            assert_eq!(
                group.get("timeout").map(Value::as_u64).flatten(),
                (kind != "load-balance").then_some(5000)
            );
            assert_eq!(
    ...
    fn input_validation_rejects_duplicates_limits_and_out_of_range_options() {
        ...
        assert!(validate(
            "fallback",
            &list,
            &options(Some(3600), Some(30), Some(1000))
        )
        .is_ok());
        for invalid in [
