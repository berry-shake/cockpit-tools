...
pub async fn status(account_id: &str) -> Result<RuntimeStatus, String> {
    ...
    drop(state);
    async fn selected(
        reader: Option<codex_proxy_engine::SelectionReader>,
    ) -> Option<codex_proxy_engine::ProxySelection> {
        match reader {
    ...
    result.sidecar_selection = if !direct {
        result.account_selection.clone()
    } else {
        s.clone()
    };
