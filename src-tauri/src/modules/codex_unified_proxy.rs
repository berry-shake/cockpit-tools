...
use crate::modules::{
    account, atomic_write, codex_proxy_catalog_binding as binding, logger, secure_account_storage,
};
...
static READ_LOCK: LazyLock<Arc<tokio::sync::Mutex<()>>> =
...
fn read_store(path: &Path) -> Result<UnifiedProxyState, String> {
    let metadata = match fs::symlink_metadata(path) {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
            return Ok(UnifiedProxyState::default())
        }
    ...
    if stored.version != STORE_VERSION
        || !matches!(stored.mode.as_str(), MODE_OFF | MODE_ALL_ACCOUNTS)
    {
        return Err(STORAGE_ERROR.into());
    }
...
pub async fn ensure_loaded() -> Result<UnifiedProxyState, String> {
    ...
    let read_guard = tokio::time::timeout(
        std::time::Duration::from_secs(4),
        READ_LOCK.clone().lock_owned(),
    )
    .await
    .map_err(|_| "UNIFIED_PROXY_TIMEOUT")?;
    if let Ok(state) = current() {
        return Ok(state);
    }
    ...
    .await
    .map_err(|_| "UNIFIED_PROXY_TIMEOUT".to_string())
    .and_then(|result| result.map_err(|_| STORAGE_ERROR.to_string()))
    .and_then(|result| result);
    if result.is_err() {
        logger::log_warn("[UnifiedProxy] 统一代理状态读取失败，保留错误并等待重试");
    }
...
fn publish_unlocked(mut next: UnifiedProxyState) -> Result<UnifiedProxyState, String> {
    let previous = STATE
        ...
        .state
        .as_ref()
        .map_or(0, |state| state.updated_at);
...
pub fn reference() -> Result<Option<Reference>, String> {
...
fn resync_commit(
    ...
    if !next.active()
        ...
        || next
            .reference
            .as_ref()
            .is_none_or(|reference| reference.source_id != source_id)
    {
        return Ok(false);
    }
    if removed {
...
async fn finish_resync(
    ...
    let Ok(Ok(permit)) =
        tokio::time::timeout(RESYNC_COMMIT_TIMEOUT, RESYNC_COMMIT_LOCK.acquire()).await
    else {
        logger::log_warn("[UnifiedProxy] 重新同步仍在写入，本次等待超时，请重试来源更新");
        return;
    };
    let result = tokio::time::timeout(
        RESYNC_COMMIT_TIMEOUT,
        tokio::task::spawn_blocking(move || {
...
pub async fn resync_source(source_id: &str, removed: bool) {
    let Ok(observed) = ensure_loaded().await else {
        return;
    };
    if !observed.active()
        || observed
...
mod tests {
    ...
    fn read_error_preserves_last_snapshot_without_misreporting_off() {
        ...
        let previous = UnifiedProxyState {
            mode: MODE_ALL_ACCOUNTS.into(),
            reference: Some(reference()),
            snapshot: Some("cockpit-proxy://saved".into()),
            ..Default::default()
        };
        ...
        assert_eq!(
            cache.state.as_ref().unwrap().snapshot.as_deref(),
            Some("cockpit-proxy://saved")
        );
    ...
    fn late_disk_result_cannot_replace_a_completed_write() {
        ...
        cache.publish(UnifiedProxyState {
            updated_at: 123,
            ..Default::default()
        });
    ...
    async fn failed_initial_read_can_retry_after_file_recovery_without_restart() {
        ...
        let restored = Stored {
            ...
            mode: MODE_ALL_ACCOUNTS.into(),
            reference: Some(reference()),
            snapshot: Some("cockpit-proxy://restored".into()),
            ..Default::default()
        };
        fs::write(&path, serde_json::to_string(&restored).unwrap()).unwrap();
        assert!(ensure_loaded().await.unwrap().active());
        assert_eq!(
            effective_snapshot().unwrap().as_deref(),
            Some("cockpit-proxy://restored")
        );
    }
